...
ind_data_wide2 <- subset(ind_data_wide, !is.na(gdp_growth_lag)) %>%
  arrange(yr_num)
ind_imp <- complete(mice(ind_data_wide2, m = 1, print = FALSE), 1)
...
# these results are very similar to what we get with the ridge regression approach, but they lose in interpretability
